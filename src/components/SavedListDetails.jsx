...
function SavedListDetails() {
  ...
  const handleToggleItem = async (itemId) => {
    try {
      const response = await api.patch(`/${listId}/items/${itemId}/toggle`);
  ...
  const handleAddItem = async (newItem) => {
    try {
      const response = await api.post(`/${listId}/items`, newItem);
  ...
  const handleUpdateList = async () => {
    ...
    if (newListName !== null) {
      try {
        ...
        const response = await api.put(`/${listId}`, {
  ...
  const handleUpdateItem = async (itemId, updatedData) => {
    try {
      const response = await api.put(`/${listId}/items/${itemId}`, updatedData);
      setItems(
        items.map((item) =>
